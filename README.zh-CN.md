...
- `docs/release-hotfix.md`：中英文发布、Hotfix、回滚和审批流程。
- `docs/service-onboarding.md`：新服务接入应用和 GitOps 模板的 checklist。
- `docs/environment-entry.md`：Dev、Test、Staging、Prod 的准入和退出标准。
- `docs/prod-change-runbook.md`：生产变更执行、验证和回滚 runbook。
...
- 发布和 Hotfix 应遵循 `docs/release-hotfix.md`，包括不可变镜像晋级和明确的回滚负责人。
- 新服务首次生产晋级前，应按 `docs/service-onboarding.md` 完成接入检查。
- 环境晋级应使用 `docs/environment-entry.md` 作为证据 checklist。
- 生产变更应遵循 `docs/prod-change-runbook.md`，并在生产 PR 中记录验证证据。
