...
本蓝图已经补充一组可直接改造使用的治理文件。正式启用前，需要把示例 GitHub team 替换成真实组织和团队：

- `.github/CODEOWNERS`：定义蓝图、应用示例、GitOps 模板和 ADR 的评审责任。
- `.github/pull_request_template.md`：统一文档、平台和模板变更的 PR 检查项。
- `.github/dependabot.yaml`：为 GitHub Actions 和示例应用依赖配置自动更新。
- `docs/governance/release-hotfix.md`：中英文发布、Hotfix、回滚和审批流程。
- `docs/guides/service-onboarding.md`：新服务接入应用和 GitOps 模板的 checklist。
- `docs/governance/environment-entry.md`：Dev、Test、Staging、Prod 的准入和退出标准。
- `docs/runbooks/prod-change.md`：生产变更执行、验证和回滚 runbook。
- `docs/adr/`：架构决策记录索引、模板和第一条已接受 ADR。
- `LICENSE`：采用 MIT 许可证，便于作为平台蓝图复用。
...
- CODEOWNERS 应把应用代码、GitOps 环境目录、平台 workflow 和生产路径映射到明确负责的团队。
- PR 模板应要求填写验证证据、发布影响、回滚说明和环境范围。
- Dependabot 只能通过正常 PR 评审更新 GitHub Actions 和依赖，不能绕过 CI。
- 改变仓库边界、晋级策略、生产控制或安全基线的架构决策，应记录到 `docs/adr/`。
- 发布和 Hotfix 应遵循 `docs/governance/release-hotfix.md`，包括不可变镜像晋级和明确的回滚负责人。
- 新服务首次生产晋级前，应按 `docs/guides/service-onboarding.md` 完成接入检查。
- 环境晋级应使用 `docs/governance/environment-entry.md` 作为证据 checklist。
- 生产变更应遵循 `docs/runbooks/prod-change.md`，并在生产 PR 中记录验证证据。
...
- 平台运营手册、服务接入手册和验收标准。
- 生产变更 runbook、环境准入 checklist 和服务接入 checklist。
